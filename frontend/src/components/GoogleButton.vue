<script setup>
const props = defineProps({
  next: { type: String, default: '/' },
  label: { type: String, default: 'Sign in with Google' }
})

// a plain link, not fetch - the oauth handshake needs a real page navigation
const href = `/auth/google?next=${encodeURIComponent(props.next)}`
</script>

<template>
  <a class="gbtn" :href="href">
    <svg class="gbtn__logo" viewBox="0 0 18 18" aria-hidden="true">
      <path
        fill="#4285F4"
        d="M17.64 9.2c0-.64-.06-1.25-.16-1.84H9v3.48h4.84a4.14 4.14 0 0 1-1.8 2.72v2.26h2.92c1.7-1.57 2.68-3.88 2.68-6.62Z"
      />
      <path
        fill="#34A853"
        d="M9 18c2.43 0 4.47-.8 5.96-2.18l-2.92-2.26c-.8.54-1.84.86-3.04.86-2.34 0-4.32-1.58-5.03-3.7H.94v2.33A9 9 0 0 0 9 18Z"
      />
      <path
        fill="#FBBC05"
        d="M3.97 10.72a5.4 5.4 0 0 1 0-3.44V4.95H.94a9 9 0 0 0 0 8.1l3.03-2.33Z"
      />
      <path
        fill="#EA4335"
        d="M9 3.58c1.32 0 2.5.45 3.44 1.35l2.58-2.58C13.46.9 11.43 0 9 0A9 9 0 0 0 .94 4.95l3.03 2.33C4.68 5.16 6.66 3.58 9 3.58Z"
      />
    </svg>
    <span>{{ props.label }}</span>
  </a>
</template>

<style scoped>
.gbtn {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  gap: 12px;
  padding: 12px 24px;
  border-radius: 999px;
  border: 1px solid #dadce0;
  background: #fff;
  color: #3c4043;
  font-family: var(--sans);
  font-size: 0.98rem;
  font-weight: 600;
  box-shadow: var(--shadow-sm);
  transition: box-shadow 0.12s ease, background 0.12s ease;
}

.gbtn:hover {
  text-decoration: none;
  background: #f8f9fa;
  box-shadow: var(--shadow-md);
}

.gbtn__logo {
  width: 18px;
  height: 18px;
  flex: 0 0 18px;
}
</style>
